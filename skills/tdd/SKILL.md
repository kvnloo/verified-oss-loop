...
The unit command is detected at onboard time (`bash tests/smoke.sh`). If that placeholder is still `unknown`, detect the tree (`pyproject.toml`, `package.json`, `Cargo.toml`, `go.mod`) and pin the command in the PR. Do not copy another project's test runner.
