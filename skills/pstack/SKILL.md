...
oss-factory does not own these files. They live in the pstack plugin. Child repos that already installed pstack keep it: extra `skills/how`, `.cursor/skills/*`, plugin settings are `local` in `.verified-oss-loop/inventory.yml` and are never overwritten.
