...
Rollout scheme (`rollout.yml`): Arch-style `rolling` by default. `staged` or `stable` for slower repos. See the kit's [docs/rollout.md](https://github.com/kvnloo/verified-oss-loop/blob/main/docs/rollout.md). Git channels are not Pages folders: `preview` the branch ≠ `/preview/`.
