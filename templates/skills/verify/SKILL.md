...
1. **Unit** — `{{UNIT_CMD}}` on the touched surface.
2. **TDD** — red command, then green command (`skills/tdd/SKILL.md`).
3. **Mutation** — `{{MUTATOR_CMD}}` if that is not `n/a`. A surviving mutant is a missing assertion. If the stack has no mutator, write `n/a`. Do not invent a score.
4. **Runtime** — `{{RUNTIME_CMD}}`. If the project has no runtime check, write `n/a` and say what you did not run.
